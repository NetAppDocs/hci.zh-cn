...
NOTE: NetApp Hybrid Cloud Control需要最新的管理服务包。在将计算节点上的固件升级到版本2.154.1之前、您必须升级到最新的管理服务包。如果要将计算节点上的固件升级到版本2.154.1、则需要使用管理服务2.2才能继续。
...
NOTE: 在升级到版本2.154.1之前、请确保检查当前的ESXi驱动程序版本。如果此驱动程序已过期，请先升级此驱动程序。然后、将计算节点的计算固件升级到版本2.154.1。请访问 link:task_hcc_upgrade_compute_node_drivers.html["更新计算节点驱动程序"^] 有关更新计算节点驱动程序的说明，其中包括 ESXi 驱动程序。
