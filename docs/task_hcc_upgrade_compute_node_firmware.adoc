...
NOTE: 要使 NetApp Hybrid Cloud Control 能够执行未来的固件升级， H300E ， H500E 和 H700E 计算节点需要 Redfish 许可证。请联系 NetApp 支持部门手动安装许可证，直到新的计算固件捆绑包在后续版本中自动执行此过程为止。
...
* * BMC 凭据 * ：更新 NetApp Hybrid Cloud Control 用于连接到计算节点 BMC 的凭据。您可以使用 NetApp Hybrid Cloud Control 之一执行此操作 link:task_hcc_edit_bmc_info.html#use-netapp-hybrid-cloud-control-to-edit-bmc-information["用户界面"] 或 。在升级之前更新 BMC 信息可刷新清单，并确保管理节点服务能够识别完成升级所需的所有硬件参数。
...
NOTE: 在运行运行运行状况检查并继续固件升级之前，必须使 ESXi 主机退出锁定模式。请参见 link:https://kb.netapp.com/Advice_and_Troubleshooting/Hybrid_Cloud_Infrastructure/NetApp_HCI/How_to_disable_lockdown_mode_on_ESXi_host["如何在 ESXi 主机上禁用锁定模式"] 和  有关详细信息 ...
...
.. 选择要升级的集群。
.. 单击 * 浏览 * 上传从下载的升级软件包https://mysupport.netapp.com/site/products/all/details/netapp-hci/downloads-tab["NetApp 支持站点"^]。
.. 等待上传完成。进度条会显示上传状态。
...
| 升级期间发生错误。 | 发生严重错误时，升级将失败并显示此通知。单击 * 下载日志 * 链接以下载日志以帮助解决此错误。解决此错误后，您可以尝试重新升级。 
...
.. 单击 * 发布 /nodes/｛ hardware_id ｝ /upgrades* 。
.. 单击 * 试用 * 。
...
... 解决集群问题后，根据需要重新进行身份验证，然后单击 * 发布 /nodes/｛ hardware_id ｝ /upgrades* 。
... 重复上述升级步骤中的步骤。
...
.. 在 REST API UI 中，单击 * 获取​ / nodes​ / ｛ hardware_id ｝​ /upgrades* 。
.. （可选）输入日期和状态参数以筛选结果。
.. 在参数字段中输入硬件主机资产 ID （` "hardwareId"` Saved from a previous step ）。
.. 单击 * 试用 * 。
.. 单击 * 执行 * 。
...
.步骤
. 浏览到 https://mysupport.netapp.com/site/downloads["NetApp 软件下载"] 页面上，单击 * NetApp HCI * ，然后单击正确版本的 NetApp HCI 的下载链接。
. 接受最终用户许可协议。
...
.. 单击 * 确定 * 。主机上的 VM 将迁移到另一个可用主机。虚拟机迁移可能需要一些时间，具体取决于需要迁移的虚拟机数量。
+
...
. 打开控制台后，单击 * 虚拟介质 > 虚拟存储 * 。
. 在 * 虚拟存储 * 屏幕上，单击 * 逻辑驱动器类型 * ，然后选择 * ISO 文件 * 。
+
image:BIOS_H410C_iso.png["显示了用于选择 ISO 文件的导航路径。"]

. 单击 * 打开映像 * 以浏览到下载 ISO 文件的文件夹，然后选择 ISO 文件。
. 单击 * 插件 * 。
. 当连接状态显示 `Device# ： VM Plug-in OK ！！` 时，单击 * 确定 * 。
. 按 * F12* 并单击 * 重新启动 * 或单击 * 电源控制 > 设置电源重置 * ，以重新启动节点。
...
. 重新登录到 BMC 用户界面，然后单击 * 系统 * 以在启动到操作系统后验证 BIOS 版本和构建时间。如果升级已正确完成，您将看到新的 BIOS 和 BMC 版本。
+
...
. 使用凭据登录。
. 单击 * 远程控制 > 启动 KVM （ Java ） * 。
. 在控制台窗口中，单击 * 介质 > 虚拟介质向导 * 。
+
image::bmc_wizard.gif[从 BMC UI 启动虚拟介质向导。]

. 单击 * 浏览 * 并选择计算固件 ` .iso` 文件。
. 单击 * 连接 * 。此时将显示一个弹出窗口，指示成功，底部将显示路径和设备。您可以关闭 * 虚拟介质 * 窗口。
+
image::virtual_med_popup.gif[显示 ISO 上传成功的弹出窗口。]

. 按 * F12* 并单击 * 重新启动 * 或单击 * 电源控制 > 设置电源重置 * ，以重新启动节点。
. 重新启动期间，按 * F11 * 选择启动选项并加载 ISO 。
. 从显示的列表中选择 * AMI Virtual CDRO* ，然后单击 * 输入 * 。如果列表中未显示 AMI 虚拟光盘，请转到 BIOS 并在启动列表中启用它。保存后，节点将重新启动。在重新启动期间，按 * F11 * 。
+
...
. 如果与控制台断开连接，请选择 * 远程控制 * 并单击 * 启动 KVM* 或 * 启动 KVM （ Java ） * 以重新连接并验证节点何时完成启动备份。您可能需要多次重新连接，以验证节点是否已成功启动。
+
