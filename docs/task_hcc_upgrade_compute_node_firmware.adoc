...
* *最终用户许可协议(EULA)*：从管理服务2.2开始、您必须先接受并保存EULA、然后才能使用NetApp Hybrid Cloud Control UI或API升级计算节点固件：
+
.. 在Web浏览器中打开管理节点的IP地址：
+
[listing]
...
*  NetApp Hybrid Cloud Control UI to upgrade a compute node （建议）
...
NOTE: 不再支持使用 NetApp Hybrid Cloud Control UI 升级 H300E/H500E/H700E 计算节点上的计算固件。要进行升级，建议您使用 ,USB 驱动器 或 ,BMC 用户界面 挂载计算固件包。
...
成功上传并验证文件后，将显示一条屏幕消息。验证可能需要几分钟时间。升级完成后，您可以下载日志。有关各种升级状态更改的信息，请参见  status changes。
