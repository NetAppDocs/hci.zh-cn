...
summary: 在 NetApp HCI 或 SolidFire 存储系统升级过程中，您可以升级适用于 vCenter Server 的 Element 插件。 
...
* * vCenter Server* ：您的 vCenter 插件 4.x 版已注册到 vCenter Server 。在注册实用程序（`https://[management node IP] ： 9443` ）中，单击 * 注册状态 * ，填写必要字段，然后单击 * 检查状态 * 以验证 vCenter 插件是否已注册以及当前安装的版本号。
* * 管理服务更新 * ：您已更新 https://mysupport.netapp.com/site/products/all/details/mgmtservices/downloads-tab["管理服务包"] 到最新版本。vCenter 插件的更新通过管理服务更新进行分发，这些更新是在 NetApp HCI 和 SolidFire 全闪存存储的主要产品版本之外发布的。
...
NOTE: 如果您使用的是 HTTP 或 HTTPS 服务器（非公开站点）或修改了 ZIP 文件名或网络设置，则可以单击 * 自定义 URL* 自定义 URL 。有关自定义 URL 的其他配置步骤，请参见适用于 vCenter Server 的 Element 插件文档，了解有关修改内部（非公开站点） HTTP 服务器的 vCenter 属性的信息。
...
在使用 VMware vCenter Server 6.7U1 升级到 VCP 4.8 后，如果 NetApp Element 配置的 * 集群 * 和 * QoSSIOC 设置 * 部分未列出存储集群或显示服务器错误，请参见 link:https://review.docs.netapp.com/us-en/vcp_2.19_vcp4.8_release_vcp_repo/vcp_reference_troubleshoot_vcp.html#errors-after-vcp-4-8-upgrade-with-vmware-vcenter-server-6-7u1["适用于 vCenter Server 的 Element 插件"^] 有关对这些错误进行故障排除的文档。
